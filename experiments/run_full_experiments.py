#!/usr/bin/env python3
"""
Script to run full EMOD experiment suite on Modal.
This runs all experiment combinations as specified:
- 40 epochs
- Two datasets: IEMOCAP_Final and IEMOCAP_Filtered
...
# Create Modal app
app = modal.App("emod-full-experiments", image=image)
...
def train_text_model(text_model_name, dataset_name, num_epochs=40):
    """Train a text model with the specified configuration"""
    import os
    import sys
    import torch
    import pandas as pd
    ...
    # Configuration
    print(f"Starting text model training: {text_model_name} on {dataset_name} for {num_epochs} epochs")
    ...
    files_in_volume = os.listdir(DATA_VOLUME_PATH)
    print(f"Files in data directory: {files_in_volume}")
...
def main():
    print("Running full EMOD experiment suite:")
    print(f"- Models: {TEXT_MODELS}")
    print(f"- Datasets: IEMOCAP_Final, IEMOCAP_Filtered")
    print(f"- 40 epochs each")
    
    # Run experiments for each combination
    experiments = []
    
    for dataset in ["IEMOCAP_Final", "IEMOCAP_Filtered"]:
        for text_model in TEXT_MODELS:
            experiment_key = f"{dataset}_{text_model}"
            ...
            print(f"Experiment {experiment_key} launched")
    
    print("\nAll experiments have been launched!")
    print("Results will be stored in the Modal volume")
    print("You can check the progress on the Modal dashboard") 