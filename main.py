...
BATCH_SIZE = 32
LEARNING_RATE = 1e-4  # Increased learning rate for better learning
RANDOM_SEED = 42
...
# Model configuration
CONFIG = {
    "num_epochs": 20,  # Significantly increased for better convergence
...
def main(force_train=False, evaluate_only=False, visualize=False,
    ...
    if should_train:
        ...
        param_groups = [
            {'params': audio_model.parameters(), 'lr': CONFIG["learning_rate"] * 2.0},  # Increased for audio model
            {'params': text_model.parameters(), 'lr': CONFIG["learning_rate"]},  # Standard for text model
            {'params': fusion_model.parameters(), 'lr': CONFIG["learning_rate"] * 5.0}  # Much higher for fusion
        ]
        ...
        class WeightedMSELoss(nn.Module):
            ...
            def forward(self, pred, target):
                ...
                # Apply higher weights to non-zero values in the target
                # This helps with class imbalance by focusing more on non-neutral emotions
                weights = torch.ones_like(target)
                weights[target != 0] = 3.0  # Higher weight for non-zero values
